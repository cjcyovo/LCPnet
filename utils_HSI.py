...
import torch
import random
def tuple_to_numpy_array(tuple_2d, dtype=None):
    """
    将二维元组转换为NumPy数组
    ...
    """
    if dtype:
        return np.array(tuple_2d, dtype=dtype)
    else:
        return np.array(tuple_2d)


def MatrixTrans(x ,batch_size, channel_size):
    """
    将将矩阵拆分为两个矩阵，相乘等于原矩阵
    ...
    for n in range(batch_size):
        ...
        result2 = []
        if channel_size==1:
            U, S, Vt = np.linalg.svd(x[n], full_matrices=False)
            n = 5
            # 截取前n个奇异值，得到近似的分解
            U_k = U[:, :n]
            S_k = np.diag(S[:n])
            ...
            # 重构分解后的两个矩阵
            matrix1 = np.matmul(U_k, S_k)
            matrix2 = Vt_k
            result1.append(matrix1)
            result2.append(matrix2)
        else:
            for c in range(channel_size):
                # 进行SVD分解
                U, S, Vt = np.linalg.svd(x[n][c], full_matrices=False)
                n = 5
                # 截取前n个奇异值，得到近似的分解
                U_k = U[:, :n]
                S_k = np.diag(S[:n])
        ...
        result4.append(result2)

    return result3,result4


def MatrixMul(a,b,c,d):
    result = np.matmul(np.matmul(a,d),np.matmul(c,b))
    return result
...
def compute_imf_weights(ground_truth, n_classes=None, ignored_classes=[]): #处理不平衡数据集
...
def camel_to_snake(name): #将输入的驼峰命名法字符串转换为蛇形命名法
