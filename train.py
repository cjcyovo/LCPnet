...
import math
from model import LDGnet
# from minimodel import presnet
# from twostreamminimodel import LDGnet
...
#                     help='the path to load the data')
# parser.add_argument('--source_name', type=str, default='IndianaSD',
#                     help='the name of the source dir')
# parser.add_argument('--target_name', type=str, default='IndianaTD',
#                     help='the name of the test dir')
...
group_train.add_argument('--alpha', type=float, default=0.5,
...
def  train(epoch, model, num_epoch, label_name, label_queue):
    ...
    for i in range(1, num_iter):
        ...
        if i % args.log_interval == 0:
            ...
            print('loss: {:.6f},  loss_cls: {:.6f},  loss_coarse2d: {:.6f}, loss_coarse3d: {:.6f}'.format(
                loss.item(), loss_cls.item(), loss_coarse2d.item(), loss_coarse3d.item()))
...
def test(model, label_name):
    ...
    with torch.no_grad():
        for data, label in test_loader:
            ...
            loss_coarse2d,loss_coarse3d, label_src_pred = model(data, text, label)
            # loss_coarse2d, label_src_pred = model(data, text, label)
            pred = label_src_pred.data.max(1)[1]
            pred_list.append(pred.cpu().numpy())
            label_list.append(label.cpu().numpy())
            loss += F.nll_loss(F.log_softmax(label_src_pred, dim=1), label.long()).item()
            loss_coarse += loss_coarse2d.item()
            loss_coarse += loss_coarse3d.item()
...
if __name__ == '__main__':
    ...
    # hyperparams_train.update({'flip_augmentation': False, 'radiation_augmentation': False, 'mixture_augmentation': False})
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        if t_correct > correct:
            ...
            print(classification_report(np.concatenate(pred), np.concatenate(label), target_names=LABEL_VALUES_tar,digits=6))

        print('source: {} to target: {} max correct: {} max accuracy{: .2f}% kappa{: .2f}\n'.format(
            args.source_name, args.target_name, correct, 100. * correct / len_tar_dataset, 100. * kappa))
